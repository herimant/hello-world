...
from animals import animals_https
...
async def a_1(message: types.Message, state: FSMContext):
    ...
    if message.text == "И кто же это?":
        str_anim = animals_https[mystate['Parametr']][0]
        animal[message.chat.username] = animals_https[mystate['Parametr']][0]
        str_http_anim = animals_https[mystate['Parametr']][2]
        photo_animal[message.chat.username] = animals_https[mystate['Parametr']][2]
        kb = [
            [
...
async def a_2(message: types.Message, state: FSMContext):
    ...
    if message.text == "Написать отзыв":
        ...
        http = animals_https[mystate['Parametr']][1]
        response = requests.get(
            f'{http}', verify=False)
        soup = BeautifulSoup(response.text, 'html.parser')
        Text_of_animal = soup.find('div', class_='dp-content-inner')
        bot_list_of_msg = []
        if animals_https[mystate['Parametr']][0] == "Обыкновенный шакал" or animals_https[mystate['Parametr']][0] == "Японский макак":
